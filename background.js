...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.type === "ACTIVATE_CONVERTER") {
    ...
    const converterFullUrl = "https://shakeeb-sa.github.io/multi-format-link-converter/";
    ...
    if (sender.tab.url.includes(converterUrlPart)) {
        ...
        chrome.tabs.query({url: "*://shakeeb-sa.github.io/multi-format-link-converter/*"}, (tabs) => {
            if (tabs.length > 0) {
                const tab = tabs[0];
                chrome.windows.update(tab.windowId, {focused: true});
